...
describe('Product Order Controller methods', () => {
    ...
    it('should add to ProductOrder if order/product not found', async () => {
        ...
        const po1 = await productOrderCtl.updateWithProduct(mock.request({quantity: 2}, {orderId: 2, productId: 1}), mock.response())
        const po2 = await productOrderCtl.updateWithProduct(mock.request({quantity: 3}, {orderId: 2, productId: 2}), mock.response())
    ...
    it('should update ProductOrder if order/product found', async () => {
        ...
        const products = await productCtl.create(mock.request(tmpProducts), mock.response())
        const po1 = await productOrderCtl.updateWithProduct(mock.request({quantity: 2}, {orderId: 2, productId: 1}), mock.response())
        const po1Updated = await productOrderCtl.updateWithProduct(mock.request({quantity: 3}, {orderId: 2, productId: 1}), mock.response())
